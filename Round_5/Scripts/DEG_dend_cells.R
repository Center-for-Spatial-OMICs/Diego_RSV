...
### Create scObj for dendritic cells based on our general cell annotation ------
...
markers <- c(
  cell_types$Dendritic_Cells$`Plasmacytoid_dendritic_cells_(pDCs)` %>% unlist() %>% as.vector(),
  cell_types$Dendritic_Cells$`Conventional_dendritic_cells_(cDCs)` %>% unlist() %>% as.vector()
)

table(rownames(All_Sobj@assays$Nanostring$counts) %in% markers)
intersect(rownames(All_Sobj@assays$Nanostring$counts), markers)
# Bst2 (pDCs) 
# "Cd8a"  "Itgam" "Itgax" (cDCs)
...
# Let me share with him the dendritic subclustering I did so maybe they can annotate it based on the 1k panel we have for his data 
...
DotPlot(obj, 
        ...
        features =  unique(c(top_markers$feature, "Bst2", "Cd8a" , "Itgam" ,"Itgax"))
) +
  geom_point(aes(size=pct.exp), shape = 21, colour="black", stroke=0.5) +
  scale_colour_viridis(option="magma") +
  guides(size=guide_legend(override.aes=list(shape=21, colour="black", fill="white"))) +
...
obj_sub <- subset(obj, Resolved_clusters %in% c("9_res_0.5",
                                                "8_res_0.5",
                                                "7_res_0.5"))

# Plotting only good dendritic subcluster markers - adding Epcam and Cav1 by the end
...
## Export dendthelail subcluster top markers + 1k panel
...
saveWorkbook(
  wb,
  file = "/mnt/scratch1/maycon/Diego_RSV_CosMx/Round_5/Objects/dend_subcluster_markers_top20.xlsx",
  overwrite = TRUE
)