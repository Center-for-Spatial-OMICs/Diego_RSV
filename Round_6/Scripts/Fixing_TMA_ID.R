...
Sobj_list <- Sobj_list_TMA_review

plots_list <- list()

for (i in seq_along(Sobj_list)) {
  TMAs_1 <- unique(Sobj_list[[i]]$TMA_3)
  for (j in seq_along(TMAs_1)) {
    obj <- subset(Sobj_list[[i]], TMA_3 %in% TMAs_1[j])
    
    obj@meta.data$highlight <- "other"
    obj@meta.data[obj@meta.data$major_celltype %in% "Lymphocyte", ]$highlight <- "Lymphocyte"
    ...
    p <- ImageDimPlot(
      ...
      size = 1.5,
      axes = FALSE
    ) + ggtitle(paste0(obj$TMA_3[1], " --- ", names(table(obj$Group))))
...
pdf("/mnt/scratch1/maycon/Diego_RSV_CosMx/Round_6/Plots/snapshot_of_TMAs_split.pdf", width = 8, height = 6) 
...
object_list_1 <- SplitObject(Sobj_list_TMA_review[[1]], split.by = "TMA_3")
object_list_2 <- SplitObject(Sobj_list_TMA_review[[2]], split.by = "TMA_3")
object_list_3 <- SplitObject(Sobj_list_TMA_review[[3]], split.by = "TMA_3")
object_list_4 <- SplitObject(Sobj_list_TMA_review[[4]], split.by = "TMA_3")

object_list_binded <- c(object_list_1, object_list_2, object_list_3, object_list_4)

# Testing spatial slot (ok!)
ImageDimPlot(
  object = object_list_binded[[1]],
...
names(object_list_binded) <- names(object_list_binded) %>% make.unique()
for (i in seq_along(object_list_binded)) {
  # Assign the list element name as a metadata column for each object
  object_list_binded[[i]]@meta.data$TMA_4 <- names(object_list_binded)[i]
}


# qsave(object_list_binded, "/mnt/scratch1/maycon/Diego_RSV_CosMx/Round_6/Objects/Sobj_list_TMA_ID_fixed.qs")
